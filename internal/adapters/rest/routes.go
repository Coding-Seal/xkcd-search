...
func Api(fetcher *services.Fetcher, search *services.Search, user *services.UserService, comic *services.Comic, cfg *config.Config, ctx context.Context) http.Handler {
	...
	mux.Handle("GET /comic/{id}", http_util.Chain(base, limitOnIP)(http_util.WrapHandler(handlers.Comic(comic))))
